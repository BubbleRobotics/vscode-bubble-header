...
const getCurrentUser = () =>
  vscode.workspace.getConfiguration()
    .get('BRheader.username') || process.env['USER'] || 'marvin'
...
const getCurrentUserMail = () =>
  vscode.workspace.getConfiguration()
    .get('BRheader.email') || `${getCurrentUser()}@bubble-robotics.com`
...
export const activate = (context: vscode.ExtensionContext) => {
  const disposable = vscode.commands
    .registerTextEditorCommand('BRheader.insertHeader', insertHeaderHandler)
